...
public:
  WebRequestHandler(const char* uri, HTTPMethod method)
  : _uri(uri)
  , _method(method)
  , next(NULL)
  {
  ...
  virtual bool handle(WebServer& server, HTTPMethod requestMethod, String requestUri) = 0;
...
class FunctionRequestHandler : public WebRequestHandler {
    typedef WebRequestHandler base;

public:
    FunctionRequestHandler(WebServer::THandlerFunction fn, const char* uri, HTTPMethod method)
    : _fn(fn)
    , base(uri, method)
    {
    }
...
class StaticRequestHandler : public WebRequestHandler {
    typedef WebRequestHandler base;
